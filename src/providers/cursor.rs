    ...
    let profile = profile_id
        ...
        .ok_or_else(|| {
            CursorError::Browser(crate::error::BrowserError::CookieDatabaseNotFound {
                path: std::path::PathBuf::from(browser.label()),
            })
        })?;

    let cookie_header = match browser.keyring_application() {
        Some(application) => {
            load_cursor_cookie_chromium(&profile.cookie_db_path, application).await?
        }
        None => load_cursor_cookie_firefox(&profile.cookie_db_path)?,
    };
...
fn normalize(
    ...
    Ok(UsageSnapshot {
        ...
        identity: ProviderIdentity {
            ...
            display_name: identity.and_then(|v| v.name),
        },
    })
}
