...
- [x] **README and screenshots:** screenshots replaced with current UI (`screenshot-hero.png`, `screenshot-claude.png`, `screenshot-cursor.png`, `screenshot-settings.png`, `screenshot-accounts.png`, four theme variants); README table updated to new filenames and equal 4-column layout; Active badge, Cursor troubleshooting, and `v400` config path fixed; COSMIC Store install instructions deferred — will be added to README after store listing is live.
- [x] **AppStream screenshots:** `resources/app.metainfo.xml` updated — single default screenshot (hero) with caption; description expanded to three paragraphs; keywords extended with AI, Codex, Claude, Cursor, Panel, Applet; `appstreamcli validate` passes (404 warnings only, files not yet on `main`).
